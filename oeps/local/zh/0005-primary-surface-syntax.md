...
oep: 5
title: 主表层语法
description: Elixir 风格表层语法（.ois）作为 Osiris 唯一书写表层：文法、到 form 的映射、S 表达式记法退居内部表示。
author: MJ
status: Draft
...
Osiris 用 Elixir 风格的表层语法书写，扩展名 `.ois`。这是全面迁移而非共
存：`.ois` 是唯一的书写表层，编辑器、格式化、LSP、模板、文档全部以它为
...
原型（`explore/elixir-surface` 分支，已合并）已端到端验证：一个 `.ois`
...
`.ois` 承载 Osiris 的书写表层，由本 OEP 定义。OEP-0001 的 S 表达式记法
...
所有面向用户的材料呈现 `.ois`：`osr init` 模板、文档示例、工具片段、诊
断。工具链——格式化器、LSP、编辑器扩展、sourcemap——必须以 `.ois` 为
目标表层。既有 `.osr` 源码作为过渡输入继续编译，项目按自己的节奏手工改
写；过渡输入不再获得新的表层特性。宏书写在 R012 的 quote 映射落地前仍
用 `.osr`，落地后新宏也用 `.ois` 书写。
...
翻译失败必须报告源码行号。针对 `.ois` 单元的编译诊断必须写 `.ois` 路
径。在 reader 原生集成之前（见路线图），翻译单元内的 span 指向翻译后文
本；实现应携带行映射，使面向用户的位置落到 `.ois` 坐标。

### R012 —— 主表层尚未定义的部分

`quote`/`unquote`（Elixir `quote do … end` ↔ syntax-quote 映射）、形参
解构、`defstruct`、内嵌 provider、一般元数据属性尚不属于主表层。需要它
们的宏用 `.osr` 书写；从 `.ois` 消费这些宏则完整支持。每一项按
OEP-0000 以本 OEP 修订形式先行定义、后实现。

## 路线图

1. **加载期翻译（已完成）：** `.ois` 源码在 workspace 加载时翻译为规范
   文本进入不变的管线。`osr sketch FILE` 暴露翻译结果供查看。
   `osr init` 生成 `.oisr`。
2. **编辑器迁移（本修订）：** VS Code 扩展与 LSP 识别 `.ois`；原生
   reader 落地前诊断按行级保真落到 `.ois` 位置。
3. **原生 reader：** 翻译器升级为一等 reader，产出携带真实 `.ois`
...
- Revision 3，2026-08-01：全面迁移：`.osrx` 改为 `.ois`，成为唯一书写表
  ...
  展）以 `.ois` 为目标。（`.ox` 曾被考虑并否决：它是计量经济学语言 Ox
  的源码扩展名，其用户群与 Osiris 的量化受众重叠；`.oxr` 短暂选用后改为
  `.ois`——Osiris 的自然缩写，唯一已知占用是 OriginLab 的内部主题文
  件。）
...
- Revision 1，2026-08-01：初版：Elixir 风格表层为主（`.ois`）、一切皆
