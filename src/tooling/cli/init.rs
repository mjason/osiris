...
# `uv run osr run src/main.oisr` 会编译并执行顶层表达式。
...
fn create_starter(
    ...
    let (relative, contents) = package_module.map_or_else(
        || (PathBuf::from("main.ois"), STARTER_SOURCE.to_owned()),
        |module| {
            (
                PathBuf::from(module).join("core.ois"),
                package_starter_source(module),
            )
        },
    );
    let source = root.join(source_root).join(relative);
    // An existing starter in EITHER surface wins: creating the `.ois` twin
