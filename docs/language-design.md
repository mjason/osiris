...
本文描述 Osiris 语言、编译器、扩展系统和工具链的设计与当前实现边界。标为 v0 的能力应以仓库测试为准；尚未实现或仍待确认的选择会在文中明确标注。

文中的“必须”“应当”和“可以”分别表示强制约束、默认选择和可选能力。第 19 节集中列出尚待确认的语法和语义选择。
...
- **多语言可读**：源码可以直接使用中文标识符，也可以由 LSP 在不改动源码的前提下显示中文领域名称和数据流。
...
`match`、protocol 和 async 语义不进入最小核心，待实际用例证明后再加入。`loop/recur`、Clojure 风格序列操作和 `for` 属于隐式 `osiris.prelude` 的控制流层：它们通过卫生宏和少量有契约的 runtime intrinsic 提供，不增加 Reader 的特殊分支。

每个模块隐式导入版本化的 `osiris.prelude` 接口；`and`、`or`、`cond`、`when`、binding 条件、集合迭代和 threading macros 在 prelude 中以卫生宏或普通函数提供，不是未声明的编译器语法。项目可以在严格构建配置中关闭隐式 prelude，并改为显式 import。

`defn-` 是标准 prelude 的声明宏：它展开为普通 `defn` 并附加 `:private true` authored metadata。Osiris 的跨模块可见性仍由显式 `export` 决定，因此该标记用于 Rich Metadata/LSP 展示，不绕过或替代接口导出规则。

### 4.5 Threading macros

`->`、`->>`、`cond->`、`cond->>`、`some->`、`some->>`、`as->` 和 `doto` 是标准 prelude 中的卫生宏，不是 Rust 编译器特殊形式。
...
- `for` 是卫生宏。单一 binding 展开到 `mapv`；需要过滤、早停或多重 binding 时展开到嵌套的 `mapcatv`。`:let` 保持局部求值顺序，`:when` 丢弃当前项后继续，`:while` 在谓词首次为假时停止词法上最近的 collection binding；内层早停不会终止外层 collection。解构 pattern 使用卫生临时参数。
- `when`、`when-not`、`and`、`or`、`cond`、`cond->`/`cond->>`、`for`/`doseq` 的 `:when`/`:while` 和 `while` 条件使用 Clojure truthiness：只有 `none` 与 `false` 为假，`0`、空字符串和空集合仍为真。它们通过 `truthy*` guard 接入严格 Bool 的 `if`，因此普通 `if` 的静态 Bool 约束不被放宽。
...
- `loop` 是卫生宏，展开到 `osiris.prelude/loop*`，其参数是 callback 和初始状态值。`recur` 展开到 `osiris.prelude/recur*`，指向词法上最近的 `loop` binding 帧；没有内层 `loop` 时则指向当前 `defn`/`fn` 参数帧，并且只能出现在该目标的尾位置。HIR 检查所属函数深度、状态数量、每个状态的类型和尾位置；显式 loop 与函数级 recur 都降低为 Python runtime 的不可见 state token + `while`，不进行 Python 自调用，因此栈空间为 `O(1)`。相互递归仍使用 `trampoline`。
...
- `trampoline` 展开到 `osiris.prelude/trampoline*`。它先调用首个函数，随后反复调用返回的零参数函数，直到得到非 callable 值，适合相互递归；每次 bounce 都在同一个 Python `while` 中完成。
- `lazy-seq` 展开到一个零参数 thunk。`osiris.prelude.lazy_seq` 延迟并记忆迭代器产生的元素，支持重复遍历而不重复执行 thunk；无限序列必须由调用方按需消费。`delay` 展开到线程安全的 `Delay[T]` memoized thunk，`force`/`deref` 只求值一次并缓存成功值或异常，`realized?` 查询是否已经求值。
- `try` 是核心结构化异常表达式，支持多个 `(catch Type name ...)` 和一个 `(finally ...)`；body-only `(try expr ...)` 在 Python 端直接透传，不生成空的 `try` 语句，且 `finally` 之后不能再出现 `catch`。`catch` 的异常类型使用封闭的 Python builtin whitelist（例如 `Exception`、`ValueError`、`TypeError`），未知 nominal 名称仍然报错。`assert` 是失败分支调用 `osiris.prelude.assert_value` 的宏，抛出不会被 `python -O` 消除的 `AssertionError`，可选消息只在失败时求值。`time` 接受一个或多个 body expression，按顺序求值并返回最后值；runtime 以单调时钟输出耗时毫秒，body 异常原样传播，不伪造成功计时。
- `with-open` 是卫生宏，按源码顺序求值资源并嵌套 `try/finally`，按逆序调用 `osiris.prelude.close`；`None` 资源跳过关闭。v0 沿用 Python `finally` 的关闭异常传播规则，尚未提供 JVM/Clojure 的 suppressed-exception 聚合。`future`/`future-call` 使用线程池提交零参数 thunk，`promise`/`deliver` 提供一次性交付并可通过 `deref` 的毫秒 timeout/default 读取，`future-done?`、`future-cancelled?` 和 `future-cancel` 暴露状态/取消操作；`locking` 围绕 callback 保证 acquire/release，`lock` 创建可重入锁。这些入口由显式 runtime ABI 支持并以未知 effect 标记异步边界。`^:dynamic` 顶层 Value 与 `binding` 已通过 `contextvars` map ABI 实现：动态读取保留声明类型，binding 初值按源码顺序在安装上下文前求值，嵌套/异常路径恢复旧值，`future` 复制提交点上下文。`with-bindings`、`with-local-vars`、`with-redefs` 仍需更完整的 Var/root mutation ABI，暂不伪装成词法 `let`。
- `map`/`mapv`、`mapcat`/`mapcatv`、`filter`/`filterv`、`reduce` 和 `fold` 是 prelude 的数据流 primitive。v0 的这些入口是 eager：`mapv`/`mapcatv`/`filterv` 物化为 Python tuple，list 变体物化为 Python list；`map`/`mapcat`/`mapv`/`mapcatv` 都接受一个或多个 collection，按最短输入停止，callback 参数按 collection 顺序对齐；需要按需计算时显式使用 `lazy-seq`。动态 callback 边界也沿用 Clojure truthiness（只有 `none`/`false` 被 `filter` 丢弃），typed HIR 仍要求静态 `filter` callback 返回 `Bool`。`reduce` 接受 `(reduce f coll)` 或 `(reduce f init coll)`，`fold` 是明确初值的同义入口。它们的 callback effects、temporal 和 data summaries 在 typed HIR 中合并，宏本身不重复实现数值逻辑。
...
这些形式的可读 Python 映射是稳定 runtime ABI（例如 `loop`、`recur`、`trampoline`、`lazy_seq`、`Reduced`、`reduced`、`reduced_p` 和 `unreduced`）。binding/threading 宏使用的 `truthy`、`is_nil`、`present` 和 `nonempty` 是小型内部 ABI：HIR 分别赋予 `A -> Bool`、`A -> Bool`、`Option[T] -> T` 和 `Sequence[T] -> Bool` 的静态行为，宏负责建立 guard 和单次求值。`for :while` 和 `doseq :while` 的 `for_stop` 是私有停止 token，只由当前 `mapcatv`/`doseq` runtime loop 消费；HIR 把 token 产生式标为 `Never`，使普通分支类型不被控制 token 污染。编译器只为这些入口提供最小类型契约；扩展包可以在其上继续定义更高层宏。

### 4.5.2 Clojure 控制流覆盖矩阵与分期

下表按“是否已经有可测试的 Osiris 语义”划分 Clojure 常用控制流。**已实现**表示当前版本可直接写入 `.osr`，并由宏展开、HIR 和 Python runtime 测试共同锁定；**运行时接入中**表示表面形式可以先冻结，但需要线程/动态绑定 ABI 完成后才允许进入稳定 prelude；**后续序列层**表示不改变编译器核心，优先作为 `osiris.prelude` 的小型 eager/lazy runtime 函数补齐；**延期**表示不能用词法 `let` 或普通函数假装实现。
...
| 迭代与数据流 | `for`（多 binding、`:let`、`:when`、`:while`）、`doseq`、`map`/`mapv`、`mapcat`/`mapcatv`、`filter`/`filterv`、`reduce`/`fold`、`reduced` 协议、`lazy-seq`、`cons`、`concat`/`lazy-cat`、`count`/`first`/`rest`/`next`/`nth`/`seq`/`empty`、`empty?`/`seq?`/`coll?`/`sequential?`、`take`/`drop`、`take-while`/`drop-while`、`keep`/`keep-indexed`、`remove`/`removev`、`distinct`/`dedupe`、`partition`/`partition-all`/`partition-by`、`interleave`/`interpose`、`take-last`/`drop-last`、`map-indexed`、`iterate`/`repeat`/`repeatedly`/`cycle`/`sequence`、`reductions`、`run!`、`doall`/`dorun`（含可选计数）、`some`/`every?`/`not-every?`/`not-any?`、`delay`/`force`/`realized?` | 无 | `transduce`/`eduction` | 完整 Clojure Seq/ChunkedSeq/Transducer protocol（当前 runtime 以明确的 `Iterable`/memoized `LazySeq` ABI 为边界） |

序列层当前采用两条明确边界：`map`/`filter`/`reduce` 等数据处理入口保持 eager 容器结果；其余序列组合返回线程安全、记忆化的 `LazySeq`，并由 `sequence` 显式把任意 `Iterable` 接入该协议。窗口与尾缓冲组合已经在这一 ABI 上完成，下一层是独立的 transducer/chunk protocol。每个新增入口必须同时声明：是否 eager、是否保持输入 identity、空值/`none` 规则、callback 求值次数、返回容器类型以及对 `reduced` 的响应；不能只凭 Clojure 函数名映射到 Python `list`。
...
核心 `if` 的条件在静态类型已知时必须是 `Bool`。标准 prelude 的 `and`、`or`、`cond`、`when`、`when-not`、`if-not`、`while` 以及 `for`/`doseq` guard 接受任意静态值，并先通过有类型契约的 `truthy*` 降低为 `Bool`；它们采用 Clojure truthiness，只有 `none` 和 `false` 为假，`0`、空字符串和空集合仍为真。`if-let`/`when-let` 使用同一 truthiness，`if-some`/`when-some` 则只把 `none` 视为空。普通 `if` 不会隐式采用这些规则。
...
`Option[T]` 表示可能不存在的值，运行时直接映射为 `T | None`，不引入 `Some` wrapper；目标 Python 3.9 使用等价的 `Optional[T]` 标注。`Float` 中的 NaN 仍然是浮点值，不能等同于 `none`。DataFrame null、NaN 和 backend-specific missing value 必须由数据接口分别建模。
...
- prelude 为每个 operator 定义稳定 operator id，并内置 scalar instances。
...
- `abs` 虽然表面是普通 prelude 函数，也可以使用相同的封闭 capability 选择 Series/Array instance；alias 仍解析到该 canonical binding，不建立第二套分派。
- instance 来自依赖 wheel 时仍只是第 8.8.1 节的 declared contract；严格 causal/schema 区域能否把其 summaries 用作证明，继续由本地 trust policy 决定。
...
以上 Python 示例使用 `dataclass(slots=True)`，要求 Python 3.10 以上。若项目目标仍为 Python 3.9，codegen 必须生成兼容的 `__slots__` 表达或省略该非语义优化；不能输出目标解释器无法解析的代码。
...
直接核心声明把整个声明 form 的 metadata 归到 declaration，把 name、parameter 和 field syntax 上的 metadata 分别归到对应实体。声明宏没有隐式复制规则：它通过 `(meta &form)` 读取调用点，再用 `with-meta`/`vary-meta` 明确附着到生成的声明或第 8.9 节的静态记录。`osr expand`、`osr inspect` 和 LSP 使用可被同一 reader 再读入的规范化 `^{...}` 形式显示 authored metadata。
...
- `:doc`：字符串或按 BCP 47 locale 分组的字符串 map；声明中的 docstring 是默认 locale 的 `:doc` 语法糖。
...
target-python = "3.9"
strict = true
extensions = ["osiris-data-ext", "osiris-polars"]
build-groups = ["osiris"]

[[tool.osiris.trust.contract]]
distribution = "osiris-data-ext"
semantic-interface-hash = "sha256:0123456789abcdef..."
ids = [
  "osiris.data.normalize",
  "osiris.data.schema-check",
]
```

`src/foo/bar.osr` 默认映射到 Osiris 模块 `foo.bar` 和 Python 产物 `foo/bar.py`。模块声明必须与文件路径一致。Python 发行版名称和 Python/Osiris 模块名称是两个概念，不得混用。

`[tool.osiris.trust]` 是 dependency contract 唯一的项目级信任入口，不由扩展自身填写。每项以 distribution、当前 `semantic-interface-hash` 和精确 contract ids 三者共同授权；加载接口 hash 不匹配时必须 fail closed，不能按包名继续信任。编译器自带标准库 trust root 由 compiler ABI 固定，项目源码中由当前编译器直接推导的 facts 不需要 allowlist。

编译器把规范化 trust 表、compiler trust root 和实际解析到的 interface hashes 计算为 `trust-policy-hash`。该 hash 必须进入 `check`/`compile`/build metadata、typed HIR 与 diagnostic query cache key、LSP workspace state 和增量失效条件；修改 trust policy 后不能复用旧的 verified facts。v0 不提供不进入该 hash 的隐式环境变量或临时全局 trust 开关。`osr inspect` 可以显示待授权的 hash/id，但不代替用户编辑项目配置，也不变成包管理命令。
...
本文的 **effective dependency graph** 是：当前项目模块、`[tool.osiris].extensions` 显式启用的扩展根，以及它们通过 runtime import、公开接口引用和扩展 marker 到达的 `.osri` 传递闭包。具体 distribution/version/extra/环境 marker 必须来自当前 target 下已验证的 `uv.lock`；编译器不得扫描环境中偶然安装但不可达的 wheel。仅被 `import-for-syntax` 到达、且没有向当前模块展开 public static record 的宏工具包不进入 runtime registry index。相同 interface 经多条路径到达时去重；同一 interface id 同时解析到不同 distribution version、wheel hash 或 semantic hash 时必须 fail closed。
...
是否把依赖 contract 用作严格 causal/schema proof 由消费者本地 trust policy 决定。标准库 trust root 或项目明确允许的 distribution + interface hash + contract id 可以在 typed HIR pass 中被提升；未受信任 contract 仍可作为普通互操作假设使用，但在要求证明的区域必须降级为 declared/unknown 并产生诊断。`osiris.toml`、`.osri` 或包内 metadata 不能声明自己受信任，启用扩展也不等于信任其安全结论。
...
`extern` 是显式 declared FFI contract，不会因为写在源码或 `.osri` 中就自动受信任，也不能为了方便伪造确定的返回类型；是否在严格证明区域提升它由第 8.8.1 节和第 10.2 节的本地 trust policy 决定。例如任意 `Any` 输入的 `np.asarray` dtype/rank 都不确定，必须使用静态 instance/overload、显式 dtype 参数和 transfer contract，或保守返回动态类型。长期可以从高质量 `.pyi` 生成这些声明，但第一版不要求 Rust 编译器理解完整 Python typing 生态。
...
uv export --locked --no-default-groups --group osiris \
  --no-emit-project --output-file target/osr/build-constraints.txt
uv build --python 3.9 \
  --build-constraints target/osr/build-constraints.txt
```

### 14.2 静态扩展标记

包含 Osiris 编译接口或宏的 wheel 在 `<distribution>.dist-info/osiris.toml` 中放置唯一静态 marker，并将其写入 wheel `RECORD`。
...
records_hash = "sha256:0123456789abcdef..."
...
- Python 依赖仍以标准 `Requires-Dist` 为准，不在 `osiris.toml` 重复声明。
...
发布 Osiris 库或扩展时，`uv build` 必须通过最小 `osiris-build` PEP 517 backend 在隔离的临时 staging 目录调用 `osr`。backend 必须先验证 effective dependency graph 的 static-record/index 合并，再生成任何文件；这项全图门禁不代表把依赖 records 嵌入当前 wheel。wheel 必须包含生成的 `.py`、公开 `.osri`、包级 `py.typed`、选择发布的 source map，以及存在 public records 时从本 distribution 接口生成的纯数据 record manifest，并全部写入 wheel `RECORD`；sdist 必须包含 `.osr` 源码、构建配置和从 sdist 重建 wheel 所需的静态资源。普通 Python backend 不会自动收集 `target/osr`，因此该 build integration 属于 v0 发布能力，不能只依赖手工预编译。

```toml
[build-system]
requires = ["osiris-build==0.1.0"]
build-backend = "osiris_build"
```

`osiris-build` 必须实现 `get_requires_for_build_wheel`：读取标准 `[project]` 表的 `dependencies` 和 `[tool.osiris].build-groups`，在已校验为最新的 `uv.lock` 中找到对应锁定项，并返回精确的 PEP 508 requirements。该 hook 只投影锁文件，不自行解析版本；依赖缺失、范围不匹配或 lock 过期必须失败。PEP 517 frontend 随后把这些接口/宏依赖安装进隔离 build environment；backend 不能假设根环境已经 `uv sync`。

`[build-system].requires` 应精确固定 `osiris-build`，后者依赖 ABI 匹配的 `osr` 编译器。规范的 `uv` 工作流还必须把同一 `uv.lock` 导出的 requirements 文件传给 `uv build --build-constraints`，约束 backend 自身的隔离安装，并以 `uv build --python <target>` 选择与 `[tool.osiris].target-python` 相同 major/minor 的 build interpreter。backend 在开始编译时再次核对实际 `sys.version_info`、目标 Python、lock fork、已安装 compiler/extension 版本和 lock hash，任一不符都必须失败，不能从同一 lock 因构建解释器不同而选择出不同接口依赖。
...
`osiris-build` 只负责标准构建钩子、依赖需求桥接、编译 staging 和 wheel 内容，不自行解析或安装依赖；依赖解析仍由 `uv`/PEP 517 frontend 完成。`osiris` 编译器自身可以继续使用 maturin 构建，不要求改用该 backend。
...
产物默认写入独立 build 目录，不污染源码目录。`osr compile` 在同一次原子 staging 中聚合当前 distribution 的 public records 并生成 sidecar；具体默认目录仍待确认。
...
LSP 不是独立的语义实现，而是 Rust compiler database 的一个前端。
...
Agent 不应直接依赖 `.osri` 的物理 S-expression 格式。compiler database、LSP 自定义查询和 `osr inspect --format json` 应返回版本化的同一结构化 symbol/semantic model；Agent 以 `binding id + document version + source span` 定位修改，不按中文字符串全局替换。返回值必须明确区分不可信 authored metadata、schema-checked static records、dependency-declared facts 和当前本地信任配置下的 compiler-verified facts。
...
osr inspect SYMBOL [--locale LOCALE] [--format text|json]
osr lsp
```
...
- `expand` 输出可读宏展开结果。
- `inspect` 从源码/compiler database 或 `.osri` 输出名称、metadata、static records/index claims、签名、declared summaries 和当前 trust context 下的 verified summaries，不 import/执行目标 Python 包；JSON 形式供 Agent 和其他工具稳定消费。
- `lsp` 启动复用编译器前端的语言服务器。

格式化命令可以在 CST 稳定后加入。Python package/wheel 构建继续由 `uv build` 驱动，并由 `osiris-build` PEP 517 backend 完成编译 staging；`osr` 不提供包解析、安装、锁定或发布命令。
...
6. Clojure 风格 prelude 控制流和数据流：完整 threading 宏组、truthy 条件宏、`nil?`/`some?`、`if-not`/`when-not`、`if-let`/`when-let`、`if-some`/`when-some`、`when-first`、`case`/`condp`、`doseq`、`dotimes`、`while`、`assert`、`throw`/`comment`/`time`、`try/catch/finally` 边界、`with-open` 资源清理、`for` 多 binding/`:let`/`:when`/`:while`、`loop/recur`（O(1) 栈）、`trampoline`、`lazy-seq`、`delay`/`force`/`deref`/`realized?`、`future`/`future-call`/`promise`/`deliver`/`locking`、`map`/`mapv`/`mapcat`/`filter`/`reduce`/`fold`，以及 `cons`/`concat`/`lazy-cat`、惰性窗口与序列组合、`run!`/`doall`/`dorun`、`some`/`every?` 系列和 `reduced`/`reduced?`/`unreduced` 提前终止协议。
...
9. public names、authored/records/declared/verified 分层、本地 contract trust policy 和宏 phase-1 IR 打包。
...
12. 最小 `osiris-build` PEP 517 backend，确保 `.py`、`.osri`、`py.typed` 和 public records 的 canonical JSON runtime manifest 进入 wheel。
13. LSP/Agent 最小闭环：诊断、双语补全/hover、跳转、宏展开、中文 semantic view 和结构化 `inspect` 查询。
...
- formatter、自动 traceback remap 和调试器集成。
...
以下决定会直接影响 parser、类型系统或生成代码，应在实现对应模块前冻结。

| 主题 | 推荐选择 | 需要确认的问题 |
| --- | --- | --- |
| 空值字面量 | `none` | 是否更偏好 Lisp 风格 `nil`？ |
| 运行时 code-as-data | v0 仅 phase 1 保留 Symbol/Keyword 身份，运行时 keyword 降为 `str` | 是否需要独立运行时 `Symbol`/`Keyword` 值？ |
| 函数类型标注 | `[[name Type] ...] -> Return` | 是否改用更紧凑的 `[name Type ...]` 或 `:returns`？ |
| 函数语义摘要 | 普通函数推断，公开 higher-order/extern 显式声明 | effect/temporal/data contract 的表面标注采用关键字子句还是独立声明？ |
| 条件类型 | 核心 `if` 严格要求 `Bool`；prelude 控制流显式使用 Clojure truthiness | 是否还需要单独提供 opt-in 的 Python truthiness 边界？ |
| 标识符映射 | `-` 转 `_`、`?` 使用确定后缀，并检测冲突 | 谓词生成名采用 `_p`、`is_` 还是其他规则？ |
| 集合语义 | Python 原生容器，struct 默认不可变 | `[]` 应映射可变 `list` 还是逻辑不可变 `Vector`？ |
| 字段访问 | `value.field` | 是否只保留 Lisp 调用形式，避免 reader 中的 dot 语法？ |
| import 语法 | `(import ...)` 与 `(py/import ...)` 分开 | 是否需要用户显式写 `import-for-syntax`，还是只供宏作者使用？ |
| metadata runtime | v0 完整支持 Clojure reader 与 phase-1 `meta` API，phase 0 默认擦除 | 是否存在必须在生成 Python 运行时读取 metadata 的首批场景？ |
| `.osri` 格式 | 规范化、可读 S-expression | 是否从第一版就采用二进制格式？ |
| Python 基线 | 保持当前包声明的 Python 3.9 | 是否提高到 3.11/3.12，以简化 dataclass 和 typing codegen？ |
| 运行时检查 | 仅 `defstruct check` 和显式边界 wrapper | 是否默认验证所有公开函数参数？ |
| build 目录 | `target/osr/` | 是否采用项目内其他固定目录？ |
| DataFrame/axes | 核心预留类型能力，完整检查后置 | 是否要求 v0 就完成 schema/axes inference？ |
| 局部赋值 | v0 不提供 `set!` | Python 互操作是否要求第一版支持显式可变局部变量？ |
