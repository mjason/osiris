...
`displayLocale` 是封闭枚举：`"zh-CN"` 表示简体中文，`"en"` 表示英文。它只决定 LSP hover、completion 和 signature help 在 Rich Metadata 中优先选择的本地化 label/doc，不改变 binding identity、类型检查或生成的 Python。请求显式携带的 locale 优先于项目值；项目未配置时使用 LSP 全局默认值。
