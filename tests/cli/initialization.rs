...
"#,
    ...
    let starter = project.join("src/main.ois");
...
"#,
    ...
    let starter = project.join("src/new_ext/core.ois");
...
"#,
    ...
    assert!(fixture.directory.join("src/main.ois").is_file());
}
...
fn init_existing_uses_the_configured_source_root() {
    ...
    assert!(output.status.success());
    assert!(fixture.directory.join("lisp/main.ois").is_file());
    assert!(!fixture.directory.join("src/main.ois").exists());
}
